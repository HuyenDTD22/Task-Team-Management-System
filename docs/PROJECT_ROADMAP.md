...
> Target: Solo Developer | Timeline: ~21 tuần (có thể điều chỉnh)
...
Phase 6: Dashboard & Notifications [Week 16-18] ████████████░░░░░░  PRIORITY 1 ⭐
Phase 7: Docker, CI/CD & Deploy    [Week 19-21] ██████████████████  ✅ DONE
```

**Tổng MVP**: ~21 tuần (làm nghiêm túc 4-6 giờ/ngày)
**Tối thiểu cho CV**: Phase 0-3 + Phase 7 (Deploy) (~12 tuần)
...
## Phase 6: Dashboard, Notifications & Advanced Features ✅ COMPLETED
**Timeline**: Week 16-18 | **Priority**: HIGH | **Difficulty**: ⭐⭐⭐

**Completed**: Real dashboard stats (`activeTaskCount`, `overdueTaskCount`, `doneTaskCount`) fetched from `GET /api/v1/users/me/stats` via 3 JPQL count queries on `TaskRepository`. Upcoming Tasks panel (top 5 by due date, reuses `useMyTasks`). Quick Actions navigate to `/workspaces` and `/tasks`. Full notification system: V10 migration (`notifications` table with partial index on unread), `NotificationService` hooked into `TaskService.assignTask()`, `CommentService.addComment()`, `SprintService.startSprint()/completeSprint()`, `ProjectService.addProjectMember()` — all wrapped in try/catch so notification failure never rolls back parent transaction. Self-notification prevention rules applied. REST API: `GET /notifications` (paginated + `isRead` filter), `GET /notifications/unread-count`, `PATCH /notifications/{id}/read` (ownership check), `PATCH /notifications/read-all`. Frontend: bell icon in sidebar with red unread badge (polling every 30s), `/notifications` page with All/Unread filter toggle + pagination + "Mark all as read", click navigates to task or project entity.

**Post-testing fixes (5 issues)**: (1) Bell repositioned inside `<nav>` below My Tasks. (2) `WORKSPACE_MEMBER_ADDED` notification added — new `NotificationType`/`NotificationEntityType` enum values, hook in `WorkspaceService.addMember()`, frontend icon + navigation. (3) Workspace member removal now cascades: guards last-project-MANAGER (WS_005 409), unassigns tasks across workspace, deletes all project memberships, then removes workspace membership. (4) Remove project member now auto-unassigns tasks in that project; `TaskDetailPanel` ghost-option fix ensures correct assignee display even for removed members. (5) Dashboard `Task Distribution` donut chart (recharts) showing TODO/IN_PROGRESS/IN_REVIEW/DONE slices; `UserStatsResponse` extended with 3 new count fields.

### Mục tiêu
Hoàn thiện toàn bộ product features trước khi deploy production: Dashboard thực với dữ liệu thật, hệ thống Notification, và các advanced features có giá trị CV cao.
...
##### Backend
- [x] `GET /api/v1/users/me/stats` — aggregate endpoint: `activeTaskCount`, `overdueTaskCount`, `doneTaskCount`

##### Frontend
- [x] Cập nhật `DashboardPage.tsx`: fetch stats thật, thay thế hardcoded `'—'`
- [x] "Upcoming Tasks" panel — top 5 tasks sắp đến hạn (reuse `useMyTasks`, `sortBy=dueDate&sortDir=asc&size=5`)
- [x] Quick Actions → navigation links thật (bỏ `disabled` + "Soon" badge)

#### 6.2 Notification System

##### Backend
- [x] Migration V10: bảng `notifications` (`id`, `user_id`, `type`, `title`, `message`, `entity_id`, `entity_type`, `is_read`, `created_at`)
- [x] `NotificationService.createNotification()` — hook vào TaskService (assignTask), CommentService (addComment), SprintService (start/complete), ProjectService (addMember)
- [x] `GET /api/v1/notifications` (paginated, filter `isRead`)
- [x] `GET /api/v1/notifications/unread-count`
- [x] `PATCH /api/v1/notifications/{id}/read`
- [x] `PATCH /api/v1/notifications/read-all`

##### Frontend
- [x] Bell icon trong `AppLayout` sidebar với unread count badge (polling 30s)
- [x] `/notifications` page với All/Unread filter, pagination, mark all as read
- [x] `useNotifications` + `useUnreadNotificationCount` hooks với `refetchInterval: 30_000`
- [x] Mark as read on click + navigate to entity (task or project)

#### 6.3 (Optional) Additional Features
...
**Recommendation**: Ưu tiên **Swagger/OpenAPI docs** — impact cao, implement nhanh, không cần infrastructure mới.

### CV Talking Points
*"Built a real-time notification system with polling-based push delivery, and a data-driven dashboard aggregating cross-project task metrics. Integrated Swagger/OpenAPI for comprehensive API documentation."*

---

## Phase 7: Docker, CI/CD & Deployment ✅ COMPLETED
**Timeline**: Week 19-21 | **Priority**: HIGH | **Difficulty**: ⭐⭐⭐

**Completed**: Full Docker production stack với GitHub Actions CI/CD. 8 files được tạo mới, không thay đổi code phase 1–6. Critical fix phát hiện: `AppProperties.cookieSecure = true` (default) gây lỗi HTTP cookie → fix bằng `APP_COOKIE_SECURE: "false"` env var trong docker-compose.yml.

### Mục tiêu
Containerize toàn bộ application, thiết lập CI/CD pipeline tự động, và deploy lên AWS EC2 production.

### Tasks

#### 7.1 Dockerize
- [x] Backend `Dockerfile` (multi-stage build) — đã có sẵn từ trước
- [x] Frontend `Dockerfile` (multi-stage: node:20-alpine → nginx:alpine)
- [x] `docker-compose.yml` (production 4-service stack)
- [x] `docker-compose.dev.yml` (PostgreSQL only — đã có sẵn từ trước)
- [x] `nginx/nginx.conf` (reverse proxy với cookie passthrough)
- [x] `frontend/nginx.conf` (SPA routing + asset caching)
- [x] `frontend/.dockerignore`
- [x] `.env.example` đầy đủ tất cả env vars
- [x] Test: `docker compose up` — tất cả services hoạt động

#### 7.2 AWS EC2 Setup
- [ ] Launch EC2 instance (t3.small, Ubuntu 22.04) — xem `DEPLOYMENT.md`
- [ ] Configure Security Groups (22 SSH, 80 HTTP)
- [ ] Install Docker Engine + Compose plugin
- [ ] Clone repo, tạo `.env` file
- [ ] `docker compose pull && docker compose up -d`
- [ ] Verify: `curl http://<EC2_IP>/api/v1/actuator/health`

#### 7.3 GitHub Actions CI/CD
- [x] `.github/workflows/ci-cd.yml` — 3-job pipeline
  - [x] `test`: PostgreSQL service container + `mvn test`
  - [x] `build-and-push`: GHCR images (`:sha-<7char>` + `:latest`) với GHA layer cache
  - [x] `deploy`: SSH via `appleboy/ssh-action@v1`, pull + compose up + prune
- [ ] Thêm GitHub Secrets: `EC2_HOST`, `EC2_SSH_KEY`, `GHCR_USERNAME`

#### 7.4 (Optional) HTTPS
- [ ] Domain setup
- [ ] Certbot Let's Encrypt SSL
- [ ] Nginx HTTPS config + `app.cookie-secure=true`
...
| `frontend/.dockerignore` | Loại node_modules, dist |
| `nginx/nginx.conf` | Reverse proxy config |
| `docker-compose.yml` | Production 4-service stack |
| `.env.example` | Env var template |
| `.github/workflows/ci-cd.yml` | CI/CD pipeline |
| `DEPLOYMENT.md` | EC2 setup guide |

### CV Talking Points
*"Deployed full-stack application on AWS EC2 using Docker Compose with Nginx reverse proxy, multi-stage Docker builds for optimized image sizes, GitHub Container Registry (GHCR) for image hosting, and GitHub Actions CI/CD pipeline (3-stage: test → build/push → deploy) for automated testing and deployment."*
...
Frontend → Backend APIs phải stable trước
  ↓
Dashboard/Notifications → Product features phải hoàn thiện trước deploy
  ↓
Deploy → Cần product hoàn chỉnh để deploy production
```
...
Phase 6 (Dashboard/Notif) ← cần Phase 5 (APIs phải stable)
Phase 7 (Deploy)          ← cần Phase 6 (product hoàn chỉnh)
```
...
| 6. Dashboard/Notif | 16-18 | Notification hooks | Scope creep |
| 7. Docker & Deploy | 19-21 | EC2 networking + CI/CD | Docker debugging |

**Lời khuyên**: Hoàn thiện Phase 0-6 (business features) trước khi deploy Phase 7. **MVP tối thiểu để demo**: Phase 0-3 + Phase 7. Nhưng Phase 6 (Dashboard/Notifications) hoàn thiện product và nên implement trước khi deploy production.
